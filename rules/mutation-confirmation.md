...
Applies to every skill under `skills/azdo-*` that issues any write. Read-only skills (e.g. `/azdo-fetch-tickets`) are outside this rule — they need no confirmation step because they cannot change state.
